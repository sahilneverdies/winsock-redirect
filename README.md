...
| Method | `PatchCsrFields` | `PatchCustomRoomA` | `PatchCustomRoomB` | HWID Mask | Memory Patch | Pool Bypass | ADB Inject |
|--------|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| 1 — CSR Remove | ✔ | ✘ | ✘ | ✔ | ✘ | ✘ | ✘ |
| 2 — Room A | ✘ | ✔ | ✘ | ✔ | ✘ | ✘ | ✘ |
| 3 — Room B | ✘ | ✘ | ✔ | ✔ | ✘ | ✘ | ✘ |
...
Full bidirectional TCP-level packet dump mode for blacklist analysis:

- Saves **every send/recv** payload as a timestamped `.dat` file to `dll_captured/`
- Files contain: direction (SEND/RECV), label, timestamp, length, full hex dump
- PC logo bypass (`PatchPcDetection`) still active during capture
- All other bypass modifications are disabled (pass-through to mitmproxy)
- Uses the existing traffic redirection (port 19112 → mitmproxy :8080)

Run in conjunction with normal mitmproxy mode for full stack: PC logo removal + UID control + TCP-level packet capture to identify the in-game blacklist trigger message.

**File format** (`dll_captured/YYYYMMDD_HHMMSS_sss_DIRECTION_label.dat`):
```
# DLL Packet Capture
# Direction: RECV
# Label: original
